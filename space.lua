...
function space.update_tab(gui_data, event)
  ...
  -- speed * 60 = km/s
  local platform_datas = {}
  if not storage.platforms then
    storage.platforms = {}
  end

  for index, platform_data in pairs(storage.platforms) do
    if platform_data.platform.valid then
      ---@type PlatformData
      local p = {
        index = platform_data.index,
        force_index = platform_data.force_index,
        name = platform_data.name,
        platform = platform_data.platform,
        hub_position = platform_data.platform.hub and platform_data.platform.hub.position,
        weight = platform_data.platform.weight / 1000, -- /1000 = tons
        contents = {} -- hub get_output_inventory get_contents
      }
      -- location
      if platform_data.location  then
        p.location = {"","[planet=", platform_data.location.name, "] "}
      else
        p.location = {"vtm.deep-space"} -- TODO -> from , to pictures
      end
      --schedule
      p.has_schedule = false
      if platform_data.platform.schedule then
        p.has_schedule = true
      end
      platform_datas[p.index] = p
    else
      -- remove invalid platform
      storage.platforms[index] = nil
    end
  end
  if storage.settings[gui_data.player.index].current_tab ~= "space" then
    gui_data.gui.space.badge_text = table_size(platform_datas)
    return
  end
  ...
  for _, p in pairs(platform_datas) do
